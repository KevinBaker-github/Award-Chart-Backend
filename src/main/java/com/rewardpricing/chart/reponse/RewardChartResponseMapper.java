...
public class RewardChartResponseMapper {
	...
	public static List<ResponseAwardChart> build(List<RewardPricingEntity> awardChartEntities) {
		Map<String, Map<String, Map<String, List<RewardPricingEntity>>>> map = 
				awardChartEntities.stream().collect(Collectors.groupingBy(RewardPricingEntity :: getCategory, 
						Collectors.groupingBy(RewardPricingEntity :: getRoomCategory, Collectors.groupingBy(RewardPricingEntity :: getPricingLevel))));
		System.out.println("response 1      "+ map);
		
		
		List<ResponseAwardChart> responseList = new ArrayList<>();
		for (Entry<String, Map<String, Map<String, List<RewardPricingEntity>>>> entry : map.entrySet()) {
	        ...
	        ResponseAwardChart awardChart = new ResponseAwardChart();
