...
import com.rewardpricing.chart.util.Constants;
...
public class DynamicPricingServiceImpl implements DynamicPricingService {
  ...
  public ResponseEntity<DynamicPricingResponse> getPricing(DynamicPricingRequest request) {
    ...
    // fetch entity
    Optional<RewardPricingEntity> finalStandardentity =
        this.repository.findByCategoryAndRoomCategoryAndPricingLevel(request.getCategory(),
    ...
    if (finalStandardentity.isPresent()) {
      ...
      if (floatingPointLevel > standardEntity.getPoints()) {
        ...
        Optional<RewardPricingEntity> finalBasePeakEntity =
            this.repository.findByCategoryAndRoomCategoryAndPricingLevel(request.getCategory(),
        ...
        if (finalBasePeakEntity.isPresent()) {
          ...
          if (request.getRoomCategory().equalsIgnoreCase(Constants.PREMIUM)) {
            ...
            if (floatingPointLevel > basePeakEntity.getPoints()) {
              ...
              Optional<RewardPricingEntity> finalPremiumPeakEntity =
                  this.repository.findByCategoryAndRoomCategoryAndPricingLevel(
                      request.getCategory(), Constants.PREMIUM, Constants.PREMIUM_PEAK);
              if (finalPremiumPeakEntity.isPresent()) {
                ...
                response.setCalculatedPricingLevel(
                    "PremiumPeak Pricing Level data not found for category : "
                        + request.getCategory());
          ...
          response.setCalculatedPricingLevel(
              "BasePeak Pricing Level data not found for category : " + request.getCategory());
          return new ResponseEntity<>(response, HttpStatus.NO_CONTENT);
        }
        ...
        if (request.getRoomCategory().equalsIgnoreCase(Constants.PREMIUM)) {
          ...
          response.setPremium(true);
          Optional<RewardPricingEntity> finalPremiumEntity =
              this.repository.findByCategoryAndRoomCategoryAndPricingLevel(request.getCategory(),
          ...
          if (finalPremiumEntity.isPresent()) {
            ...
            response.setCalculatedPricingLevel(
                "Premium Pricing Level data not found for category : " + request.getCategory());
            return new ResponseEntity<>(response, HttpStatus.NO_CONTENT);
          }
        } else {// not premium room type so check for occupancy threshold
          response.setPremium(false);
          if (request.getForecastedOccupancy() > request.getRewardSaverOccupancy()) {
            // if greater occupancy then standard
            response.setIsoccupancyThresholdAbove(true);
            ...
            response.setIsoccupancyThresholdAbove(false);
            Optional<RewardPricingEntity> finalRewardSaverEntity =
                this.repository.findByCategoryAndRoomCategoryAndPricingLevel(request.getCategory(),
            ...
            if (finalRewardSaverEntity.isPresent()) {
              ...
              response.setCalculatedPricingLevel(
                  "RewardSaver Pricing Level data not found for category : "
                      + request.getCategory());
              return new ResponseEntity<>(response, HttpStatus.NO_CONTENT);
            }
          }
